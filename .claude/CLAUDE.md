...
- 並列化できる調査・探索・独立した作業は、可能な限りサブエージェント(Agent ツール)に委譲して並列実行する
- サブエージェントのモデルはタスクの難易度で使い分ける
  - 簡単な作業(ファイル探索、定型的な調査、機械的な編集): haiku
  - 中程度の作業(限定的な実装、調査結果のまとめ): sonnet
  - 複雑な作業(設計、難しいデバッグ、コードレビュー): メイン会話と同じモデル(inherit)
