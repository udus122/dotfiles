...
次の順で進めてください。
  ...
  1. code-reviewer サブエージェントを起動する。メインの会話ではレビューしない
     渡すもの: リポジトリのパス、ブランチ名、比較元 (origin/main など)、変更の意図
     認証・入力の取り扱い・秘密情報に触れた変更では security-reviewer も通す

  2. 返ってきた指摘を直す。直さないものは理由を1行で残す。修正はコミットまで済ませる

  3. もう一度 PR を作成する

記録はレビューエージェントの終了時に自動で残ります。ブランチ単位なので、
指摘を直したコミットで再レビューは要りません。

サブエージェントを起動できない環境 (非対話実行、起動が明示依頼待ちのセッション)
では /code-review で代替し、その結果を記録してください。
