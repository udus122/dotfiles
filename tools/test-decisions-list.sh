...
# labeled も実物は body を返す。持たせないと、確認方法の有無を見る側が
# 本文なしと区別できず、ラベルの付いた本物まで警告に混ざる。
jq -n '[
  { number: 10, title: "ラベルの付いた本物",
    url: "https://example.invalid/10", updatedAt: "2026-09-01T00:00:00Z",
    body: "## 判断待ち\n\n- 判断してほしいこと: X\n\n## 解決の確認方法\n\nY を見る\n" },
  { number: 11, title: "ラベルはあるが確認方法が無い",
    url: "https://example.invalid/11", updatedAt: "2026-09-01T00:00:00Z",
    body: "## 何が起きたか\n\n本文だけ。節は1つも無い。\n" }
]' > "$tmp/labeled.json"
...
listed 11; check "確認方法は無くてもラベルがあれば一覧から落とさない" $?
...
# 標準エラーには種類の違う警告が並ぶので、全体ではなくその1種だけを数える。
check "落としたものが複数でも警告は1行に収める" \
  "$([ "$(grep -c '一覧に入れない' "$tmp/err")" -eq 1 ] && echo 0 || echo 1)"

grep -q '#10' "$tmp/err"
check "ラベルで拾えているものを警告に混ぜない" "$([ $? -ne 0 ] && echo 0 || echo 1)"

# 一覧には入れるが、何が起きれば解決なのかが本文に無い。黙って並べると
# 再検証が毎晩走るのに判定に届かないままになるので、番号を名指しする。
grep -q '判定できない.*#11' "$tmp/err"
check "確認方法の無い needs-decision を標準エラーで名指しする" $?

grep -q '判定できない.*#10' "$tmp/err"
check "確認方法のある needs-decision は名指ししない" "$([ $? -ne 0 ] && echo 0 || echo 1)"
