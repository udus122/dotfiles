...
2. 内容の種別を判定する
   - その日の出来事・振り返り・作業の要約・調査結果 → `journals/daily/YYYY-MM-DD/journal.md`(既存があれば追記)。ここがデフォルト
   - AGENTS.md の昇格基準を満たす知識 → `notes/kebab-case.md`(原則は既存ノートの更新。新規は将来更新する場面を言える名詞トピックのみ)
   - 読書メモ → `books/kebab-case.md`
   - 判断に迷う生データ → 当日の `journals/daily/YYYY-MM-DD/` に置く(type: capture)
3. frontmatter を付与して保存する
4. `notes/` `books/` のファイルを作成・大きく更新したら `index.md` と `log.md` も更新する(`journals/daily/` 配下は対象外。規約参照)
...
1. 当日の `journals/daily/YYYY-MM-DD/journal.md` の「## 作業ログ」に要約を追記する(何をやったか・結果・成果物へのリンク)
2. 昇格基準を満たす知識は `notes/` へ反映する(原則は既存ノートの更新)。切り出したら journal からリンクする
