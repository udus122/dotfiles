...
`references/weekly.md` の 1）。全部が0なら `since` か走査を疑う。

実態の0件なら、起票も草案も0件のまま `5. 消化する` へ進む。**0件を理由に消化まで
止めないこと。** 夜間ルーティンが自走しているワークスペースでは人間の発話が
構造的に発生せず、そこで止めると、そのワークスペースは消化されない側に固定される。
