...
# 件数とファイル名まで出す。印だけでは、読み手が中を見るまで何が在るのか分からず、
# それは結局 status を手で叩くことになる。競合の行と同じ形（先頭1件 + ほか N 件）に
# 揃えてあるので、2つの状態を同じ読み方で並べられる。
dirty=""
# -uall で追跡外のディレクトリを中のファイルまで開く。既定はディレクトリ1行に畳むので、
# スキルを丸ごと置いたまま止まっている形（いちばん起きやすい）が常に「1 件」になる。
changed=$(git -C "$repo" status --porcelain -uall 2>/dev/null)
c=$(printf '%s' "$changed" | grep -c . || true)
if [ "${c:-0}" -gt 0 ]; then
  # porcelain v1 は先頭2桁が状態、3桁目が区切り。パスは4桁目から始まる。
  first=$(printf '%s\n' "$changed" | head -n 1 | cut -c4-)
  more=""
  [ "$c" -gt 1 ] && more=" ほか $((c - 1)) 件"
  dirty=" / 未コミットの変更: $first$more"
fi

# 競合したまま止まった rebase / merge は、そのままだと上の「未コミットの変更」に
